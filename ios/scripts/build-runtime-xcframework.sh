#!/usr/bin/env bash
# 在 macOS 上把 apps/api/cmd/vantaloom-ios 编成 iOS 静态库并打成 xcframework。
#
#   apps/ios/scripts/build-runtime-xcframework.sh            # 设备 arm64
#   MIN_IOS=17.0 apps/ios/scripts/build-runtime-xcframework.sh
#
...
OUT="$RUNTIME_DIR/VantaloomRuntime.xcframework"
...
( cd "$API_DIR" && go build -trimpath -ldflags="-w" -buildmode=c-archive \
    -o "$SLICE/libvantaloom.a" ./cmd/vantaloom-ios )

[[ -f "$SLICE/libvantaloom.a" && -f "$SLICE/libvantaloom.h" ]] || {
  echo "error: go build 没有产出 libvantaloom.a / libvantaloom.h" >&2; exit 1; }

INCLUDE="$SLICE/include"
rm -rf "$INCLUDE"; mkdir -p "$INCLUDE"
cp "$SLICE/libvantaloom.h" "$INCLUDE/"
cp "$RUNTIME_DIR/module.modulemap" "$INCLUDE/"

echo "· xcodebuild -create-xcframework"
rm -rf "$OUT"
xcodebuild -create-xcframework \
  -library "$SLICE/libvantaloom.a" -headers "$INCLUDE" \
  -output "$OUT"

echo
echo "产物：$OUT"
du -sh "$OUT" "$SLICE/libvantaloom.a"
echo
echo "下一步："
echo "  cd $IOS_DIR && VANTALOOM_IOS_RUNTIME=true xcodegen generate && open Vantaloom.xcodeproj"
echo "  发布给公开仓 CI：zip -r VantaloomRuntime.xcframework.zip VantaloomRuntime.xcframework && shasum -a 256 …（见 runtime/README.md）"
